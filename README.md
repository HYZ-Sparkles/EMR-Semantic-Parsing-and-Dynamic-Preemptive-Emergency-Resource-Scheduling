...
- 每槽位：`state` (Idle/Treating/Preempted)、`current_patient_id`、`current_score`、`preempted_by`、`suspended: VecDeque`、`remaining_ticks / total_ticks`。
...
**每个 tick**：
1. 在队患者 + 被挂起患者每 2 tick 自动 +1 分（`deteriorate` 事件，挂起者的分数同步写回 `slot.suspended` 队列以保证 resume 时正确），更新堆。
2. 所有在治资源 `remaining_ticks - 1`；归零则置 Idle（`finish` 事件），同时清空 `current_score`。
3. 资源释放后，若有 `suspended` 链表头患者 → 恢复治疗（`resume` 事件），其最新分数写回 `slot.current_score`。
4. 重试分配堆顶：
   - 找同类空闲 → 分配。
...
curl -X POST http://127.0.0.1:7878/api/parse \
  ...
  -d '{"text":"患者突发咯血并伴随昏迷"}'
# 期望: matches=[{咯血,12},{昏迷,14}], score=26, desired_kind=CtScanner 或 RescueRoom
...
- **恶化每 2 tick +1 分**：模拟病情随时间线性恶化，对**在队患者**与**被挂起患者**都生效；分数上限为 `max_weight() * 2 = 40`。
- **拒绝无关键词主诉**：主诉里没有命中任何医学关键词时直接 reject 并发 `reject` 事件，比过去"以 score=1 入队"更诚实——免排队污染。
...
| 3.1 10-20 个医学习语                            | `term.rs` 19 个，权重 3-20，按致命/重症/中等/轻症四档 |
